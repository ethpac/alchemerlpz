...
alchemer_clean <- function(df, keep_test = FALSE, keep_dq = FALSE, keep_metadata = FALSE) {
  ...
  df <- df |>
    ...
    dplyr::mutate(date_submitted = lubridate::parse_date_time(date_submitted, orders = "ymd HMS", tz = timezone)) |>
  ...
  df <- df |>
    dplyr::select(response_id, date_submitted, status, tidyselect::matches("_answer|_options")) |>
