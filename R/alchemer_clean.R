...
alchemer_clean <- function(df, keep_test = FALSE, keep_dq = FALSE, keep_metadata = FALSE) {
  ...
  # unnest question cols (list-columns only)
  list_columns <- df |>
    purrr::keep(is.list) |>
    names()
  ...
  # rename variables using question titles
  df <- df |>
    dplyr::select(
      response_id,
      date_submitted,
      status,
      tidyselect::matches("_answer|_options")
    ) |>
    dplyr::rename_with(
      ~ purrr::map_chr(
        readr::parse_number(.),
        \(x) {
          questions |>
            dplyr::filter(qnum == x) |>
            dplyr::pull(clean_title, .default = NA_character_)
        }
      ),
  ...
  # only unnest list-columns to preserve metadata
  df <- df |>
    tidyr::unnest_wider(tidyselect::where(is.list), names_sep = "_") |>
    dplyr::rename_with(~ gsub("_1$", "", .x)) |>
    dplyr::mutate(
      dplyr::across(
        tidyselect::matches("\\d{5}$"),
        ~ purrr::map(.x, \(y) purrr::pluck(y, "answer"))
      )
    ) |>
    tidyr::unnest_wider(tidyselect::where(is.list), names_sep = "_") |>
    dplyr::rename_with(~ gsub("_1$", "", .x)) |>
  ...
  # ensure response_id exists at the end
  df <- df |>
    dplyr::mutate(response_id = response_id %||% id)

  # return final column order
  df |>
    dplyr::select(response_id, date_submitted, status, tidyselect::everything())
}
